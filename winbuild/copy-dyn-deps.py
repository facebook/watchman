#!/usr/bin/env python2
# This is run from windows packaging scripts to locate the dynamic deps
...
# 'dumpbin.exe' /nologo /dependents watchman.exe
EXAMPLE_OUTPUT = """
...
SYSTEM_BLACKLIST = set(  # noqa: C405
    [
        "advapi32.dll",
        "dbghelp.dll",
        "kernel32.dll",
...
if len(sys.argv) < 3:
    raise RuntimeError("copy-dyn-deps.py SRC_EXE INST_DIR [SEARCH_PATH]*")

# The executable of interest
EXE = sys.argv[1]
# Where to place the deps
INST_DIR = sys.argv[2]
# Paths to search for the deps; if found in one of these, the dep will
# be copied to INST_DIR.
SEARCH_PATH = sys.argv[3:]

# You can run this script on !windows to have it test against
# the EXAMPLE_OUTPUT at the top of this script
