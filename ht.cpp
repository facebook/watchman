...
{
  auto ht = (w_ht_t*)calloc(1, sizeof(w_ht_t));
...
static void resize(w_ht_t *ht, uint32_t newsize)
{
  ...
  auto table = (watchman_hash_bucket**)calloc(newsize, sizeof(void*));
  if (!table) {
    return;
  }
...
{
  return w_string_equal(
      (w_string_t*)w_ht_val_ptr(a), (w_string_t*)w_ht_val_ptr(b));
}
