...
static const char* type_names[] =
    {"object", "array", "string", "integer", "real", "true", "false", "null"};
...
static void
scanner_init(scanner_t* s, json_error_t* error, size_t flags, const char* fmt) {
  s->error = error;
  s->flags = flags;
  s->fmt = s->start = fmt;
  s->line = 1;
...
static int unpack_object(scanner_t* s, json_t* root, va_list* ap) {
  ...
  */
  ...
  if (root && !json_is_object(root)) {
    set_error(s, "<validation>", "Expected object, got %s", type_name(root));
    goto out;
  }
  next_token(s);
  ...
  while (s->token != '}') {
    ...
    if (s->token != 's' && s->token != 'u' && s->token != 'm') {
      set_error(s, "<format>", "Expected format string, got '%c'", s->token);
      goto out;
    }
  ...
  switch (s->token) {
    ...
    case 'm':
      if (root && !json_is_string(root)) {
        set_error(
            s, "<validation>", "Expected string, got %s", type_name(root));
        return -1;
      }
    ...
    case 'i':
      if (root && !json_is_integer(root)) {
        set_error(
            s, "<validation>", "Expected integer, got %s", type_name(root));
        return -1;
      }
    ...
    case 'f':
      if (root && !json_is_real(root)) {
        set_error(s, "<validation>", "Expected real, got %s", type_name(root));
        return -1;
      }
    ...
    case 'n':
      ...
      if (root && !json_is_null(root)) {
        set_error(s, "<validation>", "Expected null, got %s", type_name(root));
        return -1;
      }
      return 0;
