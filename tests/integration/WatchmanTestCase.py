...
class WatchmanTestCase(unittest.TestCase):
    ...
    def normAbsolutePath(self, path):
        ...
        path = path.replace('\\', '/')
        # TODO: in the near future we will guarantee that all paths
        # generated by the test fixtures are already in canonical
        # case so we will be able to remove the normcase call.
