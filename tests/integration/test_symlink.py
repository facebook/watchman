...
class TestSymlink(WatchmanTestCase.WatchmanTestCase):
    ...
    def makeRootAndConfig(self):
        ...
        with open(os.path.join(root, ".watchmanconfig"), "w") as f:
            f.write(json.dumps({"watch_symlinks": True}))
        return root
    ...
    def test_symlinkToFileDir(self):
        ...
        self.assertTrue(
            res["files"]
            and (
                "symlink_target" not in res["files"][0]
                or res["files"][0]["symlink_target"] is None
            )
        )
    ...
    def test_watchSymlinkTarget(self):
        ...
        os.symlink(os.path.join(root1prime, "file"), os.path.join(root1, "linkToPrime"))
    ...
    def test_watchRelativeSymlinkTarget(self):
        def make_roots():
            ...
            self.touchRelative(rootB, "file")
            os.symlink(
                os.path.relpath(os.path.join(rootB, "file"), rootA),
    ...
    def test_watchRelativeSymlinkTargetOneHop(self):
        def make_roots():
            ...
            os.symlink(
                os.path.relpath(
                    os.path.join(rootB, "file"), os.path.join(rootA, "dir")
                ),
    ...
    def test_watchSymlinkToDir(self):
        def make_roots():
            ...
            os.symlink(
                os.path.join(rootB, "dir", "file"), os.path.join(rootA, "linkToDir")
            )
            self.touchRelative(rootA, "file")
            return rootA, rootB
    ...
    def test_watchSymlinkToDirContainsSymlink(self):
        def make_roots():
            ...
            self.touchRelative(rootC, "file")
            os.mkdir(os.path.join(rootB, "dir"))
            os.symlink(os.path.join(rootC, "file"), os.path.join(rootB, "dir", "link"))
            os.symlink(os.path.join(rootB, "dir"), os.path.join(rootA, "linkToDir"))
