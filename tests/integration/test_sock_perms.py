...
class TestSockPerms(unittest.TestCase):
    ...
    def test_invalid_sock_access(self):
        ...
        with self.assertRaises(pywatchman.SocketConnectError) as ctx:
            instance.start()
        self.assertEqual(ctx.exception.sockpath, instance.getSockPath())
        self.assertWaitFor(lambda: 'Expected config value sock_access to be an object' in
        ...
        with self.assertRaises(pywatchman.SocketConnectError) as ctx:
            instance.start()
        self.assertEqual(ctx.exception.sockpath, instance.getSockPath())
        self.assertWaitFor(lambda: 'Expected config value sock_access.group to be a boolean' in
                      instance.getCLILogContents())
