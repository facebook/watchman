...
struct Bytestring<'a>(#[serde(borrow)]
                      Cow<'a, [u8]>);
...
struct BytestringArray<'a>(#[serde(borrow)]
                           Vec<Bytestring<'a>>);

#[derive(Debug, Deserialize, Eq, PartialEq)]
struct BytestringObject<'a>(#[serde(borrow)]
                            HashMap<Bytestring<'a>, Bytestring<'a>>);

#[derive(Debug, Deserialize, Eq, PartialEq)]
struct TwoBytestrings<'a>(#[serde(borrow)]
                          Bytestring<'a>,
                          #[serde(borrow)]
                          Bytestring<'a>);
...
fn test_newtype_variant() {
    // {"TestNewtype": "foobar"}
    let bser_v2 = b"\x00\x02\x00\x00\x00\x00\x05\x1a\x00\x00\x00\x01\x03\x01\x02\x03\x0bTestNewtype\
                    \x02\x03\x06foobar";
    let decoded = from_slice::<BytestringVariant>(bser_v2).unwrap();
    assert_eq!(decoded,
...
fn test_struct_variant() {
    ...
    let bser_v2 =
        b"\x00\x02\x00\x00\x00\x00\x05+\x00\x00\x00\x01\x03\x01\x02\x03\nTestStruct\
