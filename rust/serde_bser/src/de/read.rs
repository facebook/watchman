...
impl<'a> DeRead<'a> for SliceRead<'a> {
    ...
    fn verify_remaining(&self, len: usize) -> Result<()> {
        ...
        if actual_len < len {
            bail!("Expected at least {} bytes, but only {} bytes remain in slice",
                  len,
                  actual_len);
        }
        Ok(())
    ...
    fn verify_read_count(&self, len: usize) -> Result<()> {
        if self.index != len {
            bail!("Expected {} bytes read, but only read {} bytes",
                  len,
                  self.index);
        }
        Ok(())
    ...
    where T: ?Sized + 'b + 'c
{
    pub fn map_result<F, U, E>(self, f: F) -> Result<Reference<'b, 'c, U>>
        where F: FnOnce(&T) -> result::Result<&U, E>,
              Error: From<E>,
              U: ?Sized + 'b + 'c
    {
        match self {
